...
const sprites = {
	...
	dirt: document.getElementById('sprites-dirt')
}
...
class Bullet {
	constructor(){
		...
		this.y = player.y+(player.h-this.h)/2-((player.w-this.w)/2);
...
const ground = {
	...
	w: innerWidth,
	h: innerHeight,
	color: c.createPattern(sprites.dirt,'repeat'),
	draw: function() {
		c.beginPath();
                    ...
                    if(enemies[j].health<=0) {
                        ...
                        if(parseInt(score.innerHTML)!=0){
                            if (parseInt(score.innerHTML)%100==0) {
                                enemies.push(new Enemy(200,300,0.55,0,false,3))
                            }
                            if (parseInt(score.innerHTML)%150==0) {
                                enemies.push(new Enemy(200,300,0.25,0,false,5))
                            }
                        }
                    }
				}
